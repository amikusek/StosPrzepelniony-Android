...
import pl.sggw.stosprzepelniony.viper.chat.adapter.view_holder.InterlocutorMessageViewHolder;

import static pl.sggw.stosprzepelniony.viper.chat.adapter.item.MessageItem.SENDER_MESSAGE_TYPE;

public class InterlocutorMessageDelegate extends AdapterDelegate<List<MessageListItem>> {

    @Override
    protected boolean isForViewType(@NonNull List<MessageListItem> items, int position) {
        return items.get(position).getType() == SENDER_MESSAGE_TYPE;
    }

    @NonNull
    @Override
    protected RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent) {
        return new InterlocutorMessageViewHolder(
                LayoutInflater
                        .from(parent.getContext())
                        .inflate(R.layout.message_interlocutor, parent, false));
    }
    ...
    protected void onBindViewHolder(@NonNull List<MessageListItem> items, int position,
        ...
        ((InterlocutorMessageViewHolder) holder).bindDataObject(((MessageItem) items.get(position)).getMessage());
    }
}
